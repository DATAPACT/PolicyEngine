...
class ConflictingRule(BaseModel):
    conflicting_rules_in_p_one: Set[str] = Field(...)
    conflicting_rules_in_p_two: Set[str] = Field(...)
    conflict_explanation: Union[str, Dict[str, Any]]

class ConflictResponse(BaseModel):
    conflict_status: str
    p_one_id: str
    p_two_id: str
    p_one_role: Optional[str] = None
    p_two_role: Optional[str] = None
    conflict_set: Optional[List[ConflictingRule]] = None
                    ...
                    "permission": [
                        {
                            ...
                            "assignee": "http://example.com/party/consumer",
                            "constraint": [
                                {
                                    "leftOperand": "odrl:purpose",
                                    "operator": "odrl:eq",
                                    "rightOperand": "dpv:Research"
                                }
                            ]
                        }
                    ]
...
# Add this as a new endpoint in your FastAPI app
@app.post("/detect_conflict", response_model=ConflictResponse)
async def detect_conflict(request: DetectConflictRequest = Body(...)):
    """
    Detects conflicts between two ODRL policies.

    Takes two policies (p_one and p_two) and optionally their roles (p_one_role and p_two_role).
    Returns a detailed conflict report if conflicts are found.

    Parameters:
    - request: DetectConflictRequest containing the two policies and optional role information

    Returns:
    - ConflictResponse: Detailed information about any conflicts found
    """
    raise HTTPException(status_code=500, detail=f"Not implemented")
...
def check_policy_conflict(existing_policy: Graph, request_policy: Graph) -> Union[None, str]:
    ...
    dpv_graph = Graph()
    dpv_graph.parse("https://w3id.org/dpv/owl", format="xml")  # Or TTL if available
